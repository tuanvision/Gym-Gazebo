...
def getPoints(image):
    thresh, maxVal = processImage(image) 
...
#     mid = width/2
#     return np.rint((y - mid) * 10.5 / mid + 10)
...
if __name__ == '__main__':
    ...
    for epoch in xrange(current_epoch+1, epochs+1, 1):
        ...
        for t in xrange(steps):
            ...
            totalState = np.append(normalizedLaser, points)

            # get Q-values
            qValues = deepQ.getQValues(totalState)

            action = deepQ.selectAction(qValues, explorationRate)

            newObservation, reward, done, info = env.step(action)
            [newImage, newLaser, newPosition] = newObservation
            newPoints = getPoints(newImage)
            newNormalizedLaser = normalize(newLaser)
            newTotalState = np.append(newNormalizedLaser, newPoints)
            ...
            deepQ.addMemory(totalState, action, reward, newTotalState, done)
            ...
            if done:
                ...
                if (epoch)%20==0:
                    print ("Saving model ...")	
                    deepQ.saveModel('/home/tuanguyen/Desktop/model/turtle_mazecolor_camera_dqn'+'.h5')
                    ...
                    with open('//home/tuanguyen/Desktop/model/turtle_mazecolor_camera_dqn'+'.json', 'w') as outfile:
